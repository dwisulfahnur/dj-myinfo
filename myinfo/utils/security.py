...
def generate_client_assertion(url: str, jkt_thumbprint: str) -> str:
    ...
    jws_key = jwk.JWK.from_json(settings.MYINFO_PRIVATE_KEY_SIG)
...
def decrypt_jwe(encrypted_data: str) -> dict:
    jwe_key = jwk.JWK.from_json(settings.MYINFO_PRIVATE_KEY_ENC)
    jwetoken = jwe.JWE()
    jwetoken.deserialize(encrypted_data, key=jwe_key)

    # verify the signature of the decrypted JWS
    jwkset = get_jwkset(settings.MYINFO_JWKS_DATA_VERIFICATION_URL)
